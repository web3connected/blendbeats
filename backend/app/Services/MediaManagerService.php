...
class MediaManagerService
{
    ...
    public function getFileUrl(MediaFile $file): string
    {
        return match ($file->disk) {
            'public', 'local', 'media_s3', 's3' => "/api/media/files/{$file->id}/stream",
    ...
    public function uploadFileToMediaManager(UploadedFile $file, string $disk, ?string $collection = null): MediaFile
    {
        ...
        app(MediaStorageQuotaService::class)->assertUploadAllowed(Auth::user(), $file);

        $mediaAccount = app(MediaSetupService::class)->activeMediaAccount(Auth::user());

        if (! $mediaAccount) {
            throw ValidationException::withMessages([
                'media_account' => ['Activate your media library before uploading files.'],
            ]);
        }
        ...
        $this->auditAction('upload', $mediaAccount->disk, $path);
    ...
    public function listMediaFiles(string $disk, string $path = '', ?string $collection = null): array
    {
        $this->validateDiskAccess($disk, 'view');

        $files = MediaFile::where('disk', $disk)
            ->when(! $this->currentUserIsAdmin(), fn ($query) => $query->where('user_id', Auth::id()))
    ...
    {
        $this->validateDiskAccess($file->disk, 'view');
        $this->authorizeFileOwner($file);
